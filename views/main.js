...
async function DOMloadChat(grpid) {
    ...
    if (localChats == undefined) {
        const getAllChats = await axios.get("http://13.233.174.100:3000/getAllChats",
...
async function getUpdatedChats(grpid) {
    ...
    if (grpid == 1) {
        const result = await axios.get(`http://13.233.174.100:3000/getGroupToken/${grpid}`,
            { headers: { "Authorization": token } });
    ...
    const updatedMsg = await axios.get(`http://13.233.174.100:3000/getUpdate/${lastMsgId}`,
        { headers: { "Authorization": token } });
...
async function DOMloadGroups() {
    const token = localStorage.getItem("token");
    const getGroups = await axios.get("http://13.233.174.100:3000/getAllGroups",
        { headers: { "Authorization": token } });
...
async function onGroupClick(e) {
    ...
    if (e.target.classList.contains("open")) {
        ...
        try {
            ...
            const result = await axios.get(`http://13.233.174.100:3000/getGroupToken/${groupId}`,
                { headers: { "Authorization": token } });
...
async function sendMsg() {
    ...
    try {
        ...
        await axios.post("http://13.233.174.100:3000/send-msg", {
            msg: msg
        }, { headers: { "Authorization": token } })
...
async function createGroup() {
    ...
    try {
        ...
        const result = await axios.post("http://13.233.174.100:3000/createGroup", { groupName: groupName }, { headers: { "Authorization": token } });
...
async function addmember() {
    ...
    if (member) {
        console.log(member);
        try {
            const result = await axios.post("http://13.233.174.100:3000/addmember", {
                member: member
            }, { headers: { "Authorization": token } });
...
async function publicGroup() {
    try {
        const token = localStorage.getItem("token");
        const result = await axios.get("http://13.233.174.100:3000/getPublicToken",
            { headers: { "Authorization": token } });
        localStorage.setItem("token", result.data.token);
...
async function showpopup() {
    ...
    try {
        const token = localStorage.getItem("token");
        const allMembers = await axios.get("http://13.233.174.100:3000/viewAllMembers",
            { headers: { "Authorization": token } });
...
async function memberClick(e) {
    ...
    if (e.target.classList.contains("admin")) {
        try {
            const result = await axios.get(`http://13.233.174.100:3000/addAdmin/${userid}`,
                { headers: { "Authorization": token } });
        }
        catch (err) {
            console.log(err)
        ...
        try {
            const result = await axios.get(`http://13.233.174.100:3000/removeMember/${userid}`,
                { headers: { "Authorization": token } });
        }
        catch (err) {
            console.log(err)
